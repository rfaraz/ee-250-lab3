...
def save_mail(mail: List[Dict[str, str]]) -> None:
    ...
    Writes the mail input to a new json file
...
def add_mail(mail_entry: Dict[str, str]) -> str:
    """TODO: fill out this docstring (using the load_mail docstring as a guide)
    Loads mail file, writes to mail file, generates a unique identifier, and saves the mail entry

    Returns: 
        str: A unique id for newly added mail entry
    """
    mail = load_mail()
...
def delete_mail(mail_id: str) -> bool:
    """TODO: fill out this docstring (using the load_mail docstring as a guide)
    Loads mail file, removes each entry from loaded mail file, and resaves empty output, clearing the mail

    Returns:
        bool: A flag determining whether the operation was successful or unsuccessful
    """
    mail = load_mail()
...
def get_mail(mail_id: str) -> Optional[Dict[str, str]]:
    """TODO: fill out this docstring (using the load_mail docstring as a guide)
    Given a mail ID number, this retrieves the text of the mail file

    Returns:
        Dict: In the case that the corresponding mail file is found, the content of the mail is returned.
    """
    mail = load_mail()
    for entry in mail:
        if entry['id'] == mail_id:
...
def get_inbox(recipient: str) -> List[Dict[str, str]]:
    """TODO: fill out this docstring (using the load_mail docstring as a guide)
    Given a specified user, loads all the mail items sent to that user. 

    Returns:
        List: A list of all mail files sent to the specified user
    """
    mail = load_mail()
    inbox = []
    for entry in mail:
...
def get_sent(sender: str) -> List[Dict[str, str]]:
    """TODO: fill out this docstring (using the load_mail docstring as a guide)
    Loads all mail files sent out by a specified user.

    Returns:
        List: A list of all mail files sent out by a specified user.
    """
    mail = load_mail()
    sent = []
    for entry in mail:
